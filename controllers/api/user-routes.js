...
router.post("/login", (req, res) => {
  User.findOne({
    ...
    if (!dbUserData) {
      res
        .status(400)
        .json({ message: "the realm has no record of this email!" });
      return;
    }
    ...
    if (!validPassword) {
      res
        .status(400)
        .json({ message: "you shall not pass with incorrect passwords!" });
      return;
    }
  ...
  User.update(req.body, {
    ...
    .then((dbUserData) => {
      if (!dbUserData) {
        res
          .status(404)
          .json({ message: "no user in the realm found with this id" });
        return;
      }
      res.json(dbUserData);
    })
